...
// Presentation-only streak explanation; rules remain server-canonical.
...
function WeeklyStreak({ days, currentStreak }: { days: WellnessMomentumDay[]; currentStreak: number }) {
  return (
    <article className="rounded-[1.7rem] border border-slate-100 bg-white p-4 shadow-[0_12px_35px_rgba(15,23,42,0.07)]">
      ...
      <div className="mt-2 text-[10px] font-bold leading-4 text-slate-400">
        Streak adalah jumlah hari berhasil berturut-turut, bukan total hari target tercapai dalam satu bulan.
      </div>
      ...
      {days.length > 0 && !days[days.length - 1]?.success && currentStreak > 0 ? (
        <div className="mt-3 rounded-2xl border border-blue-100 bg-blue-50 px-3 py-2.5 text-[10px] font-bold leading-4 text-blue-800">
          Hari ini masih belum tercapai. Streak {currentStreak} hari sebelumnya tetap tersimpan dan tidak di-reset; angka akan bertambah saat target hari ini terpenuhi.
        </div>
      ) : null}
