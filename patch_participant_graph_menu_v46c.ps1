...
$root = "C:\Users\Lenovo\Documents\mcu_system_nextjs"
$page = Join-Path $root "app\wellness\portal\page.tsx"
$menu = Join-Path $root "app\wellness\portal\_components\ParticipantPortalMenu.tsx"
$chartComponent = Join-Path $root "app\wellness\portal\_components\AchievementChartsTab.tsx"
$report = Join-Path $root "graph_menu_patch_report_v46c.txt"
...
if (!(Test-Path $menu)) {
    throw "ParticipantPortalMenu.tsx tidak ditemukan: $menu"
}

Write-Host "PATCH PARTICIPANT GRAPH MENU V46C"
Write-Host "Target benar: app\wellness\portal\_components\ParticipantPortalMenu.tsx"
Write-Host "Tambah menu hamburger: Grafik"
Write-Host "Tambah component: AchievementChartsTab"
Write-Host "Tidak mengubah API, Google Sheet, Supabase, Google Fit, atau Health Connect."

# ============================================================
# 1. Patch page.tsx: import, PortalTab, render
# ============================================================

$pageText = Get-Content $page -Raw -Encoding UTF8

if (!$pageText.Contains('import AchievementChartsTab from "./_components/AchievementChartsTab";')) {
    if ($pageText.Contains('import ParticipantPortalMenu from "./_components/ParticipantPortalMenu";')) {
        $pageText = $pageText.Replace(
            'import ParticipantPortalMenu from "./_components/ParticipantPortalMenu";',
            'import ParticipantPortalMenu from "./_components/ParticipantPortalMenu";' + "`r`n" + 'import AchievementChartsTab from "./_components/AchievementChartsTab";'
        )
        Write-Host "OK - import AchievementChartsTab ditambahkan"
    } else {
        throw "import ParticipantPortalMenu tidak ditemukan di page.tsx"
    }
} else {
    Write-Host "SKIP - import AchievementChartsTab sudah ada"
}

if (!$pageText.Contains('"charts"')) {
    $portalTabPattern = '(?s)(type\s+PortalTab\s*=\s*)(.*?);'

    if ([regex]::IsMatch($pageText, $portalTabPattern)) {
        ...
        throw "type PortalTab tidak ditemukan di page.tsx"
    }
} else {
    Write-Host "SKIP - PortalTab charts sudah ada"
}

if (!$pageText.Contains('activeTab === "charts"')) {
...
'@
    ...
    if ($pageText.Contains($needle)) {
        $pageText = $pageText.Replace($needle, $renderCharts + $needle)
        Write-Host "OK - render Grafik ditambahkan sebelum History"
    } else {
        throw 'blok activeTab === "history" tidak ditemukan di page.tsx'
    }
} else {
...
# ============================================================
# 2. Patch ParticipantPortalMenu.tsx
# ============================================================

$menuText = Get-Content $menu -Raw -Encoding UTF8

# Relax / extend common tab union patterns inside component file
$menuText = $menuText.Replace('| "profile";', '| "profile" | "charts";')
$menuText = $menuText.Replace("| 'profile';", "| 'profile' | 'charts';")
$menuText = $menuText.Replace('| "profile")', '| "profile" | "charts")')
$menuText = $menuText.Replace("| 'profile')", "| 'profile' | 'charts')")
$menuText = $menuText.Replace('| "profile">', '| "profile" | "charts">')
...
if ($menuText.Contains('title: "Grafik"') -or $menuText.Contains('label: "Grafik"')) {
    Write-Host "SKIP - menu Grafik sudah ada di ParticipantPortalMenu.tsx"
    Set-Content -Path $menu -Value $menuText -Encoding UTF8
} else {
    Set-Content -Path $menu -Value $menuText -Encoding UTF8

    $lines = New-Object System.Collections.Generic.List[string]
    (Get-Content $menu -Encoding UTF8) | ForEach-Object { $lines.Add($_) }
    ...
    if ($targetLine -lt 0) {
        ...
        $diag.Add("GAGAL MENEMUKAN ITEM HISTORY DI PARTICIPANTPORTALMENU")
        $diag.Add("File: $menu")
        $diag.Add("Cari manual lokasi item menu dari hasil di bawah:")
        $diag.Add("")

        for ($i = 0; $i -lt $lines.Count; $i++) {
            $lower = $lines[$i].ToLower()
            if ($lower.Contains("history") -or $lower.Contains("riwayat") -or $lower.Contains("profile") -or $lower.Contains("devices")) {
        ...
        throw "Item History di ParticipantPortalMenu.tsx tidak ditemukan. Buka report: $report"
    }
    ...
    if ($objectStart -lt 0) {
        throw "Awal object History tidak ditemukan di ParticipantPortalMenu.tsx"
    }
    ...
    Set-Content -Path $menu -Value ($lines -join "`r`n") -Encoding UTF8
    Write-Host "OK - menu Grafik ditambahkan di ParticipantPortalMenu.tsx"
}

# ============================================================
# 3. Create AchievementChartsTab.tsx
# ============================================================
...
Set-Content -Path $chartComponent -Value $componentText -Encoding UTF8
Write-Host "OK - AchievementChartsTab.tsx dibuat / diperbarui"
...
Select-String -Path $page -Pattern 'AchievementChartsTab' -SimpleMatch | ForEach-Object {
    Write-Host ($_.LineNumber.ToString() + ": " + $_.Line.Trim())
}
Select-String -Path $page -Pattern 'activeTab === "charts"' -SimpleMatch | ForEach-Object {
    Write-Host ($_.LineNumber.ToString() + ": " + $_.Line.Trim())
}

Write-Host ""
Write-Host "VALIDATION MENU"
Select-String -Path $menu -Pattern 'Grafik' -SimpleMatch | ForEach-Object {
    Write-Host ($_.LineNumber.ToString() + ": " + $_.Line.Trim())
}
Select-String -Path $menu -Pattern 'charts' -SimpleMatch | ForEach-Object {
    Write-Host ($_.LineNumber.ToString() + ": " + $_.Line.Trim())
}

Write-Host ""
Write-Host "VALIDATION COMPONENT"
Select-String -Path $chartComponent -Pattern 'export default function AchievementChartsTab' -SimpleMatch | ForEach-Object {
    Write-Host ($_.LineNumber.ToString() + ": " + $_.Line.Trim())
}

Write-Host ""
Write-Host "DONE - PATCH PARTICIPANT GRAPH MENU V46C"