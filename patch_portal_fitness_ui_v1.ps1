...
Write-Host "PATCH PORTAL FITNESS UI V1"

$text = Get-Content $path -Raw -Encoding UTF8

# Fix return utama yang sebelumnya pernah rusak dari section orange ke main
$badReturn = @'
return (
...
if ($text.Contains($badReturn)) {
    ...
    Write-Host "OK - return utama diperbaiki"
}

# Hero utama dibuat selaras dengan fitness app pastel
$text = $text.Replace(
...
if ($end -lt 0) {
    throw "function NutritionTab tidak ditemukan setelah HomeTab."
}
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 text-3xl font-black">
                    {fmtNumber(Math.max(stepsProgress, workoutProgress, nutritionProgress), 0)}%
                  </div>
                ...
                <FitnessRing percentage={Math.max(stepsProgress, workoutProgress, nutritionProgress)} />
              </div>
...
$text = $text.Substring(0, $start) + $newHome + $text.Substring($end)

Set-Content -Path $path -Value $text -Encoding UTF8

Write-Host "OK - HomeTab diganti menjadi fitness app style"
Write-Host "DONE PATCH PORTAL FITNESS UI V1"