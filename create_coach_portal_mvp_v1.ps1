...
Write-Host "CREATE COACH PORTAL MVP V1"

$loginDir = Join-Path $project "app\api\wellness\coach\login"
$meDir = Join-Path $project "app\api\wellness\coach\me"
$dashboardDir = Join-Path $project "app\api\wellness\coach\dashboard"
$notesDir = Join-Path $project "app\api\wellness\coach\notes"
$pageDir = Join-Path $project "app\wellness\coach"
...
Set-Content -Path (Join-Path $loginDir "route.ts") -Value $loginRoute -Encoding UTF8
...
Set-Content -Path (Join-Path $meDir "route.ts") -Value $meRoute -Encoding UTF8
...
Set-Content -Path (Join-Path $dashboardDir "route.ts") -Value $dashboardRoute -Encoding UTF8
...
Set-Content -Path (Join-Path $notesDir "route.ts") -Value $notesRoute -Encoding UTF8
...
Set-Content -Path (Join-Path $pageDir "page.tsx") -Value $page -Encoding UTF8

Write-Host "DONE - Coach portal MVP files created."