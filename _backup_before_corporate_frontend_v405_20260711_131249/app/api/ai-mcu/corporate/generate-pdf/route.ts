...
function identityRow(p: any, imported: any, maps: any) {
  ...
  const mcuDate = formatDateId(pick(row["Tanggal MCU"], row.TANGGAL_MCU, row["TGL MCU"], row.TGLMCU, row["Tanggal Pemeriksaan"], p.mcu_date, row.service_date, p.service_date, p.created_at));
  ...
  return {
    ...
    FIT_STATUS: pick(row.FIT_STATUS, row.fitStatus, p.fit_status, p.status_fit),
  };
}
...
function buildParticipantRows(
  ...
  for (const code of supportCodes) {
    ...
    if (sections.has(code)) {
      support[config.resultField] = pickRowValue(selectedRaw, config.resultAliases);
      support[config.conclusionField] = pickRowValue(selectedRaw, config.conclusionAliases);
    }
