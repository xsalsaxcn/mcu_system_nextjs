...
import PortalHamburgerMenu from "./_components/PortalHamburgerMenu";

// WELLNESS_PARTICIPANT_OTP_STRAVA_GFIT_V385_PORTAL_HAMBURGER
// Participant-facing Wellness portal. Access is gated by OTP.
// Mobile workout log uses card layout. Desktop keeps table layout.
// Floating hamburger menu added for navigation back to internal dashboards.
...
export default function WellnessParticipantPortalPage() {
  ...
  const [message, setMessage] = useState(
    "Masuk menggunakan KODE/No Karyawan dan OTP."
  );
  ...
  async function loadMe(options?: { keepMessage?: boolean }) {
    ...
    if (result.ok) {
      ...
      if (!options?.keepMessage) {
        setMessage(
          "Portal peserta aktif. Activity manual, Strava, dan Google Fit akan muncul di history."
        );
      }
    }
  ...
  const totals = useMemo(() => {
    ...
    for (const item of workoutItems || []) {
      minutes += Number(item.duration_minutes || item.total_duration_minutes || 0);
      calories += Number(item.calories || item.total_calories || 0);
    }

    return {
      minutes,
      calories,
      count: workoutItems?.length || 0,
    };
  }, [workoutItems]);
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900 md:px-8">
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <section className="overflow-hidden rounded-[2rem] bg-gradient-to-r from-blue-700 via-indigo-600 to-emerald-500 p-6 pr-20 text-white shadow-xl shadow-blue-100 md:p-7 md:pr-24">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black md:text-4xl">
                Portal Peserta Wellness
              </h1>

              <p className="mt-2 max-w-3xl text-sm font-bold leading-6 text-white/90">
                Akses peserta wajib melalui OTP. Setelah masuk, peserta bisa
                konek Strava dan Google Fit, lalu activity otomatis masuk ke
                history workout.
              </p>
            </div>
            ...
            {participant ? (
              <button
                type="button"
                onClick={logout}
                className="rounded-full bg-white px-5 py-3 text-xs font-black text-blue-700"
              >
                Logout
              </button>
            ) : null}
          ...
          <section className="grid gap-6 lg:grid-cols-[1fr_360px]">
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <p className="mt-1 text-sm font-bold text-slate-500">
                Masukkan KODE/No Karyawan. Email/No HP dipakai untuk validasi
                bila data tersedia.
              </p>
            ...
            <aside className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-3 text-sm font-bold leading-6 text-slate-600">
                ...
                <p>
                  Untuk production, matikan <code>WELLNESS_OTP_DEBUG</code> dan
                  sambungkan OTP ke email/WhatsApp gateway.
                </p>
              </div>
            </aside>
          </section>
            ...
            <section className="grid gap-4 md:grid-cols-3">
              <div className="rounded-3xl border border-blue-100 bg-blue-50 p-5 text-blue-800">
                ...
                <div className="mt-2 text-xl font-black">
                  {fmt(participant?.code)} - {participant?.name}
                </div>
              ...
              <div className="rounded-3xl border border-amber-100 bg-amber-50 p-5 text-amber-900">
                ...
                <div className="mt-2 text-xl font-black">
                  {fmtNumber(totals.minutes, 1)} menit /{" "}
                  {fmtNumber(totals.calories, 0)} kkal
                </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-bold leading-6 text-slate-500">
                      Tarik jenis aktivitas, durasi, jarak, dan kalori dari Strava.
                    </p>
                  </div>
              ...
              <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm font-bold leading-6 text-slate-500">
                      Legacy Google Fit. Untuk project baru, rencanakan Health
                      Connect/native bridge.
                    </p>
                  </div>
