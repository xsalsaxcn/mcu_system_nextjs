...
export async function GET(req: NextRequest) {
  ...
  if (sourceId === "all" && keyword.length < 2 && !birthDate) {
    return ok({
      participants: [],
      message: "Pilih database, isi nama/MCU/NIK minimal 2 karakter, atau isi tanggal lahir."
    });
  }
  ...
  function baseQuery() {
    ...
    if (birthDate) query = query.or(`birth_date.eq.${birthDate},date_of_birth.eq.${birthDate}`);
  ...
  return ok({ participants: mapRows(data || []), mode: keyword.length >= 2 ? "like" : "birth_date_or_source" });
}
