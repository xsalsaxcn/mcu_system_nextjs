...
export async function GET(req: NextRequest) {
  ...
  const examDate = req.nextUrl.searchParams.get("exam_date") || "";
  const limit = Math.min(Math.max(Number(req.nextUrl.searchParams.get("limit") || 25), 1), 50);

  if (sourceId === "all" && keyword.length < 2 && !examDate) {
    return ok({
      participants: [],
      message: "Pilih database, isi nama/MCU/NIK minimal 2 karakter, atau isi tanggal MCU."
    });
  }
  ...
  function baseQuery() {
    ...
    if (examDate) query = query.or(`examination_date.eq.${examDate},exam_date.eq.${examDate}`);
  ...
  return ok({ participants: mapRows(data || []), mode: keyword.length >= 2 ? "like" : "exam_date_or_source" });
}
