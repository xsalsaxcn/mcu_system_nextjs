...
function statusBadge(status: any) {
  ...
  if (value === "CALLED") return "bg-blue-100 text-blue-700";
...
export default function VaccinationOnsiteQueuePage() {
  ...
  const [message, setMessage] = useState("Pilih session lalu aktifkan Mode Onsite Rolling QR.");
      ...
      <div className="mx-auto max-w-7xl rounded-3xl border bg-white p-5 shadow-sm md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-slate-600">Mode baru untuk walk-in onsite. Peserta scan QR rolling sekali, isi Nama Lengkap + NIK Karyawan + Email, lalu langsung mendapat nomor antrean. Satu NIK Karyawan hanya mendapat satu nomor per event.</p>
        ...
        <section className="mt-6 rounded-2xl border bg-slate-50 p-5">
          <div className="grid gap-3 lg:grid-cols-[1fr_170px_auto] lg:items-end">
            ...
            <label className="block">
              ...
              <select value={intervalSeconds} onChange={(e) => setIntervalSeconds(e.target.value)} className="w-full rounded-xl border bg-white px-3 py-3 text-sm font-bold">
                <option value="30">30 detik</option>
                <option value="45">45 detik</option>
                <option value="60">60 detik</option>
              </select>
            </label>
            <button disabled={!sessionId || busy} onClick={() => post({ action: "ensure-event", sessionId: Number(sessionId), intervalSeconds: Number(intervalSeconds), queuePrefix: "Q" })} className="rounded-xl bg-violet-600 px-5 py-3 text-sm font-black text-white disabled:opacity-50">Aktifkan Mode Onsite</button>
          </div>
        </section>
          ...
          <>
            <section className="mt-6 grid gap-5 xl:grid-cols-[380px_1fr]">
              <div className="rounded-3xl border border-violet-200 bg-violet-50 p-5">
                ...
                <div className="mt-1 text-center text-xs font-semibold text-slate-500">QR rolling {data?.rolling?.interval_seconds || data?.event?.qr_interval_seconds || 45} detik. Foto QR lama akan kedaluwarsa.</div>
              ...
              <div>
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                  ...
                  <div className="rounded-2xl border bg-white p-4"><div className="text-xs font-bold text-slate-500">In Process</div><div className="mt-2 text-4xl font-black text-indigo-700">{active.length}</div></div>
                ...
                <button disabled={busy || !waiting.length} onClick={() => post({ action: "call-next", eventId: data.event.id })} className="mt-4 w-full rounded-2xl bg-blue-600 px-5 py-4 text-base font-black text-white disabled:opacity-40">Panggil Nomor Berikutnya</button>
                  ...
                  <div className="mt-3 grid gap-3 md:grid-cols-2">
                    {active.map((entry: any) => (
                      <div key={entry.id} className="rounded-xl border border-blue-200 bg-blue-50 p-4">
                        <div className="flex items-start justify-between gap-3"><div><div className="text-3xl font-black text-blue-700">{entry.queue_number}</div><div className="font-bold text-slate-950">{entry.participant_name}</div><div className="text-xs text-slate-500">{entry.employee_id}</div></div><span className={`rounded-full px-3 py-1 text-xs font-black ${statusBadge(entry.queue_status)}`}>{entry.queue_status}</span></div>
                        <div className="mt-3 flex flex-wrap gap-2">
                          {entry.queue_status === "CALLED" ? <button onClick={() => post({ action: "start", eventId: data.event.id, entryId: entry.id })} className="rounded-lg bg-indigo-600 px-3 py-2 text-xs font-black text-white">Start</button> : null}
            ...
            <section className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs font-semibold text-amber-700">Nomor skipped tidak ikut Call Next. Jika orangnya datang, klik Aktifkan Kembali. Nomor lama dipertahankan dan kembali ke Waiting sehingga akan diproses sebelum nomor waiting yang lebih besar.</p>
                </div>
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                {skipped.map((entry: any) => (
                  <div key={entry.id} className="rounded-xl border border-amber-200 bg-white p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3"><div><div className="text-2xl font-black text-amber-800">{entry.queue_number}</div><div className="font-bold text-slate-950">{entry.participant_name}</div><div className="text-xs text-slate-500">{entry.employee_id}</div></div><span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800">SKIPPED</span></div>
            ...
            <section className="mt-6 overflow-hidden rounded-2xl border bg-white">
              <div className="border-b bg-slate-50 px-4 py-3 font-black">Waiting Queue</div>
              <div className="max-h-[520px] overflow-auto">
                <table className="min-w-full text-sm">
                  <thead className="sticky top-0 bg-slate-100 text-xs uppercase text-slate-600"><tr><th className="p-3 text-left">No</th><th className="p-3 text-left">Nama</th><th className="p-3 text-left">NIK Karyawan</th><th className="p-3 text-left">Email</th><th className="p-3 text-left">Aksi</th></tr></thead>
                  <tbody className="divide-y">{waiting.map((entry: any) => <tr key={entry.id}><td className="p-3 text-xl font-black">{entry.queue_number}</td><td className="p-3 font-bold">{entry.participant_name}</td><td className="p-3">{entry.employee_id}</td><td className="p-3 text-xs">{entry.email}</td><td className="p-3"><div className="flex gap-2"><button onClick={() => post({ action: "call", eventId: data.event.id, entryId: entry.id })} className="rounded-lg border px-3 py-1 text-xs font-bold">Call</button><button onClick={() => post({ action: "skip", eventId: data.event.id, entryId: entry.id })} className="rounded-lg border px-3 py-1 text-xs font-bold text-amber-700">Skip</button></div></td></tr>)}</tbody>
                </table>
              </div>
            </section>
          </>
