...
async function loadParticipant(p: any, mode: LoadMode) {
  ...
  return (
    <div className="space-y-5"> {savePopupStateV198 && ( <div className="fixed inset-0 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm" style={{ zIndex: 2147483647 }} data-save-popup-mobile-v198="true" role="status" aria-live="polite"> <div className="w-full max-w-[420px] rounded-[28px] border border-slate-200 bg-white p-7 text-center shadow-2xl md:p-8"> <div className={savePopupStateV198 === "success" ? "mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 text-[34px]" : "mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-blue-50 text-[34px]"}> {savePopupStateV198 === "success" ? "OK" : "..."} </div> <div className="mt-4 text-2xl font-black leading-tight text-slate-950"> {savePopupStateV198 === "success" ? "Penyimpanan berhasil" : "Penyimpanan diproses"} </div> <div className="mt-2 text-sm font-bold leading-6 text-slate-500"> {savePopupStateV198 === "success" ? "Kembali ke field pencarian..." : "Mohon tunggu, hasil pemeriksaan sedang disimpan."} </div> </div> </div> )} {/* SAVE_POPUP_REACT_OVERLAY_V198 */}
      <ScannerModal
        open={scannerOpen}
          ...
          {!capaskaProgressTotalV151(parameters) && <div className="rounded-xl bg-amber-50 p-3 text-sm text-amber-700">Tidak ada parameter untuk post ini. Jalankan SQL reference dan cek mapping package.</div>}
            ...
            <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-1 text-sm text-slate-500">
                Pilih tepat satu dokter/petugas yang melakukan tindakan pada stage ini.
              </div>
