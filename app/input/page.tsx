    ...
    async function start() {
      try {
        ...
        if (!AnyWindow.BarcodeDetector) {
          setStatus("Scanner otomatis tidak tersedia di browser ini. Masukkan barcode manual.");
          return;
        }
        ...
        setStatus("Arahkan kamera ke barcode/QR peserta.");
        ...
        setStatus("Kamera tidak bisa dibuka. Izinkan camera permission atau input barcode manual.");
      }
    }
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-4 md:items-center">
      <div className="w-full max-w-lg rounded-3xl border border-slate-700 bg-slate-950 p-4 text-white shadow-2xl">
        <div className="mb-3 flex items-center justify-between">
          ...
          <button type="button" onClick={onClose} className="rounded-xl bg-slate-800 px-3 py-2 font-bold">Tutup</button>
        </div>

        <video ref={videoRef} className="h-72 w-full rounded-2xl bg-black object-cover" playsInline muted />

        <div className="mt-3 rounded-xl bg-slate-900 p-3 text-sm text-slate-200">{status}</div>
    ...
    <div className="space-y-5">
      ...
      <section className="card p-5">
        ...
        <div className="mt-2 w-fit rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">
          AutoScore v17 aktif · scan barcode · edit hasil · daftar status
        </div>
      </section>
