...
function RegistrasiUlang({ user }: { user: any }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="card p-5 no-print">
        <form onSubmit={search} className="grid gap-3 md:grid-cols-2 xl:grid-cols-[180px_1fr_1fr_1fr_auto]">
            ...
            {results.map((p) => (
              ...
              >
                ...
                <div className="text-sm text-slate-500">
                  {p.mcu_id || p.external_id || "-"} · NIK {p.employee_nik || p.nik || "-"} · {formatDate(p.birth_date || p.date_of_birth)} · {p.source_name || "-"}
                </div>
              </button>
            ))}
          </div>
