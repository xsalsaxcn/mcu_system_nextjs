...
function RegistrasiUlang({ user }: { user: any }) {
  ...
  const [showLabelBarcodeText, setShowLabelBarcodeText] = useState(true);
  ...
  async function loadDetail(participant: Participant) {
    ...
    try {
      ...
      setMessage("Data peserta siap diedit. Setting print label mengikuti paket peserta jika sudah diatur.");
    } catch (err: any) {
        ...
        <section className="card p-5 no-print">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="text-sm text-slate-500">
                Jumlah print otomatis mengikuti setup paket jika sudah diatur. Bisa diubah manual sementara sebelum print.
              </div>
            </div>
          ...
          <div className="mt-4 rounded-xl bg-blue-50 p-3 text-sm font-bold text-blue-700">
            Total label yang akan dicetak: {printJobs.length}. Setting default per paket ada di toolbar Setup Label Paket.
          </div>
        </section>
      )}
  ...
  return (
    <div
      ...
      style={{
        ...
        border: showBorder ? "0.25mm solid #111" : "none",
        borderRadius: showBorder ? "1.2mm" : "0",
        overflow: "hidden",
    ...
    >
      <div
        ...
        style={{
          gridTemplateRows: "4.5mm 1fr 4.6mm",
          gap: "0.7mm"
        }}
      >
        <div
          className="flex items-center justify-between border-b border-black pb-[0.4mm]"
          style={{ fontSize: `${fontSize}px`, lineHeight: 1 }}
        >
          <div className="font-black tracking-wide">{station.label}</div>
          <div className="font-bold">
            {genderText} / {ageText} &nbsp; {examDate}
          </div>
        </div>

        <div className="grid min-h-0" style={{ gridTemplateColumns: showQr ? "1fr 10.5mm" : "1fr", gap: "1mm" }}>
          <div className="min-w-0">
            <div
              className="truncate font-black uppercase"
              style={{ fontSize: `${fontSize + 2}px`, lineHeight: 1.05 }}
            >
              {nameText}
            </div>

            <div className="mt-[0.8mm] grid grid-cols-[11mm_1fr] gap-x-1" style={{ fontSize: `${fontSize}px`, lineHeight: 1.08 }}>
              <div className="font-bold">No. MCU</div>
              <div className="truncate font-black">: {idText}</div>

              <div className="font-bold">NIK Kary</div>
              <div className="truncate">: {nikText}</div>

              <div className="font-bold">Tgl Lahir</div>
              <div className="truncate">: {birthText}</div>

              <div className="font-bold">Paket</div>
              <div className="truncate">: {packageText}</div>

              {departmentText && (
                <>
                  <div className="font-bold">Dept</div>
                  <div className="truncate">: {departmentText}</div>
                </>
              )}
            </div>
          </div>
          ...
          {showQr && (
            <div className="flex items-start justify-end pt-[0.5mm]">
              <QRCodeImage value={idText} size={36} />
            </div>
          )}
        </div>

        <div
          className="grid items-center border-t border-black pt-[0.5mm]"
          style={{ gridTemplateColumns: "1fr 8mm", gap: "1mm" }}
        >
          <div className="min-w-0">
            {showBarcodeText ? (
              <div
                className="truncate font-mono font-black tracking-[0.12em]"
                style={{ fontSize: `${Math.max(fontSize - 1, 7)}px`, lineHeight: 1 }}
              >
                ||||| {idText} |||||
              </div>
            ) : (
          ...
          <div
            ...
            style={{ fontSize: `${fontSize + 3}px`, lineHeight: 1 }}
          >
            {station.shortCode}
          </div>
        </div>
