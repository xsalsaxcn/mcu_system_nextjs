/**
 * WELLNESS_GOOGLE_DRIVE_FOLDER_FIX_V369_APPS_SCRIPT
...
function doGet(e) {
  try {
    return jsonResponse({
      ok: true,
      marker: 'WELLNESS_GOOGLE_DRIVE_FOLDER_FIX_V369',
...
function uploadEvidenceToDrive(payload) {
  ...
  file.setDescription('Uploaded from Harmony Health App Wellness. Marker: WELLNESS_GOOGLE_DRIVE_FOLDER_FIX_V369');
...
function getEvidenceRootFolder(folderName) {
  ...
  if (configuredFolderId) {
    try {
      ...
      throw new Error(
        'Tidak bisa mengakses WELLNESS_DRIVE_FOLDER_ID. Pastikan Script Property berisi folder ID saja, folder ada, dan Web App di-deploy dengan Execute as: Me. ' +
        'Raw value: ' + String(rawFolderId || '(kosong)') + '. Parsed ID: ' + configuredFolderId + '. Detail: ' +
...
function checkWellnessDriveConfig_() {
  ...
  if (!parsedFolderId) {
    result.message = 'WELLNESS_DRIVE_FOLDER_ID belum diisi. Script akan fallback membuat folder di lokasi spreadsheet jika upload dilakukan.';
    return result;
  }
  ...
  try {
    ...
    result.message = 'Root folder bisa diakses.';
    return result;
  } catch (err) {
    result.message = 'Root folder tidak bisa diakses: ' + (err && err.message ? err.message : String(err));
    return result;
  }
}
